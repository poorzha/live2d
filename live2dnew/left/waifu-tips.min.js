function loadWidget(config) {
    ...
    (function registerEventListener() {
      ...
      document
        ...
        .addEventListener("click", () => {
          if (window.Asteroids) {
            ...
            script.src = "https://itggg.cn/cdn/asteroids.js";
      ...
      document
        ...
        .addEventListener("click", () => {
          showMessage(["照好了吗，是不是很可爱呢？", "!砸瓦鲁多!"], 6000, 9);
    ...
    (function welcomeMessage() {
      ...
      if (location.pathname === "/") {
        ...
        if (location.hostname === referrer.hostname)
          ...
          text = `Hello!来自百度搜索的朋友<br>你是搜索<span>${
            referrer.search.split("&wd=")[1].split("&")[0]
          }</span>找到的我吗？`;
        else if (domain === "so")
          text = `Hello!来自360搜索的朋友<br>你是搜索<span>${
            referrer.search.split("&q=")[1].split("&")[0]
          }</span>找到的我吗？`;
        else if (domain === "google")
          text = `Hello!来自谷歌搜索的朋友<br>欢迎阅读<span>「${
    ...
    function showHitokoto() {
      fetch("https://v1.hitokoto.cn")
        ...
        .then((result) => {
          const text = `这句一言来自<span>「${result.from}」</span>，是<span>${result.creator}</span>在hitokoto.cn投稿的。`;
    ...
    async function loadModel(modelId, modelTexturesId, message) {
      ...
      if (useCDN) {
        ...
        console.log(`Live2D模型${modelId}-${modelTexturesId}加载完成`);
      }
    }
      ...
      if (useCDN) {
        ...
        fetch(`${apiPath}rand_textures/?id=${modelId}-${modelTexturesId}`)
          ...
          .then((result) => {
            ...
            else loadModel(modelId, result.textures.id, "我的新衣服好看吗？");
          });
      }
    }
  ...
  function initWidget(config, apiPath) {
    if (typeof config === "string") {
      config = { waifuPath: config, apiPath };
    }
    document.body.insertAdjacentHTML(
      "beforeend",
